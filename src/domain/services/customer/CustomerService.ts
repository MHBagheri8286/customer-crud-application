...
export const createCustomerService = (
  ...
  const validateAndCreateCustomer = async (
    ...
    const isValidPhone = validationService.validatePhoneNumber(
      data.phoneNumber
    );
    if (!isValidPhone) {
      errors.push({
  ...
  const validateAndUpdateCustomer = async (
    ...
    if (!existingCustomer) {
      return {
        success: false,
        errors: [{ field: 'id', message: 'Customer not found' }],
      };
    }

    // Validate phone number if provided
    if (data.phoneNumber) {
      const isValidPhone = await validationService.validatePhoneNumber(
        data.phoneNumber
      );
      if (!isValidPhone) {
        errors.push({
    ...
    if (
      ...
      !validationService.validateBankAccountNumber(data.bankAccountNumber)
    ) {
      errors.push({
        field: 'bankAccountNumber',
        message: 'Invalid bank account number format',
  ...
  const removeCustomer = async (
    id: string
  ): Promise<CustomerServiceResult<void>> => {
